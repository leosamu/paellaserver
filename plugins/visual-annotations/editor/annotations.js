...
(function() {
	...
	class AnnotationTrackPlugin extends paella.editor.TrackPlugin {
		...
		onSelect(trackItemId) {
			...
			/*this._tracks.some(function(trackItem, i) {
				console.log(trackItem);
                if (trackItem.id == trackItemId) {
                  paella.player.videoContainer.seekToTime(trackItem.s);
                }
			});	*/		
			...
			paella.player.videoContainer.currentTime()
					.then((_time) => {
			            ...
			            self._tracks.push({
						              ...
						              e: annotation['time'] + annotation['duration'],
						              name: annotation['title'],
						              data: JSON.parse(annotation.content).data.es,
	...
	app.directive("annotationsidebar", function() {
		return {
			restrict: "E",
			templateUrl:"/visual-annotations/directives/content.html",
			controller:["$scope","PaellaEditor",function($scope,PaellaEditor) {
				...
				$scope.$watchCollection('selectedAnnotation', function() {
					...
					else
					{
				        ...
				        $scope.timer = window.setTimeout( function() {
				            ...
				            annotation['type'] = $scope.selectedAnnotation.type;
				...
				PaellaEditor.subscribe($scope,() => {
					...
					$scope.currentTrack.plugin._tracks.some(function(trackItem, i) {						
		                if (trackItem.id == $scope.currentTrack.plugin._currentId) {
		                  currentTrackItem = trackItem;
		                }
					});	
					
					if (currentTrackItem){
						$scope.selectedAnnotation =currentTrackItem;//PaellaEditor.currentTrackItem.trackData;
						$scope.annotationLoad=true;				
						paella.player.videoContainer.seekToTime(currentTrackItem.s);//PaellaEditor.currentTrackItem.trackData.s);
						paella.player.play();
						paella.player.pause();
